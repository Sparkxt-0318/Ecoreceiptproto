...
Respond in JSON only:
{
  ...
  // The IE branch of the discriminated union allows null tier, 0, or 1-5.
  // Normalize both null and 0 → 5 (the model sometimes emits 0 instead of
  // null when honestly admitting "no tier" — same intent, different shape).
  // The SourcedVerdict branch's schema only permits 1-5, so this fallback
  // only fires on the IE branch in practice. Stage 7 short-circuits on
  // self-classified IE, so this normalized value never affects the
  // honesty-gate decision; it's purely for downstream type stability.
  const rawTier = v.rebuttal_source_tier;
  const tier = rawTier === null || rawTier === 0 ? 5 : rawTier;
